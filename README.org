...
: hop -f ics-anon --tz America/Montreal notes.org

- =-w/--weeks=     weeks ahead to scan (default 4)
- =-d/--event-duration=  duration for events with no end time (default 60)
- =--tz=           timezone for the output (default Europe/Paris)
